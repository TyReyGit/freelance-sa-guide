...
**Answer-level fact accuracy is not fully verified, and one confirmed error is documented here rather than hidden.** Retrieval accuracy (20/20 on the eval set) proves the right source chunk is found — it says nothing about whether the model's stated numbers are correct within that chunk. A manual spot-check against five live answers, checked against the actual source PDF pages, found four correct and one wrong: asked what interest rate SARS charges on a late provisional tax payment, the app answered "11.25% from 1 March 2025 to 30 April 2025," citing p.15 of the provisional tax guide. The cited page is correct — but the actual rate printed there is 10.25%, not 11.25%. The source's own rate sequence across the three periods is 10.25% → 11.00% → 10.25%; the app's answer was 11.25% → 11.00% → 10.25%, which reads as the model smoothing a dip in the source data into what looked like a more plausible monotonic decline. The answer also cited the wrong section for this rate (s89*quat*(2), an underpayment-interest provision with its own R50,000 threshold) instead of the correct one (s89*bis*, which actually governs late-payment interest and is what the question asked about) — while still pointing at the right page.

This is a more informative failure than a wrong citation would have been: retrieval worked, grounding worked (the number really does trace to a real page in a real source document), and the number was still wrong. It's the same class of failure as the fabricated legal citations found in `sars-dates-mcp` — a passing verification step (correct retrieval, a resolvable citation) doesn't guarantee the underlying claim is true. Neither eval harness in this project currently checks numeric values against source text at the individual-figure level; both check retrieval and citation resolution, not arithmetic or transcription fidelity within the cited passage.
